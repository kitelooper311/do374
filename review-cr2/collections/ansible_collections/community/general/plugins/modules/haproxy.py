...
class HAProxy(object):
    ...
    def discover_all_backends(self):
        ...
        return tuple(
            map(lambda d: d["pxname"], filter(lambda d: d["svname"] == "BACKEND", r))
        )
    ...
    def get_state_for(self, pxname, svname):
        ...
        state = tuple(
            map(
                ...
                filter(
                    lambda d: (pxname is None or d["pxname"] == pxname) and d["svname"] == svname,
                    r,
                ),
            )
        )
    ...
    def wait_until_status(self, pxname, svname, status):
        ...
        self.module.fail_json(
            msg="server %s/%s not status '%s' after %d retries. Aborting."
    ...
    def act(self):
        ...
        self.command_results["changed"] = (
            self.command_results["state_before"] != self.command_results["state_after"]
        )
